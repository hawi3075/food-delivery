...
const Home = () => {
  ...
  useEffect(() => {
    const getRestaurants = async () => {
      try {
        setLoading(true);
        // Using a 5000 port (ensure your backend server.js is running!)
  ...
  const handleCategoryClick = (category) => {
    ...
    if (category === 'All') {
      ...
      const filtered = restaurants.filter(res => res.category === category);
    ...
    <div className="bg-[#FDFDFD] min-h-screen pb-24">
      ...
      <div className="bg-white p-4 pt-12 sticky top-0 z-50">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="bg-gray-50 p-2.5 rounded-2xl relative border border-gray-100 shadow-sm active:scale-90 transition-all">
        ...
        <div className="grid grid-cols-1 gap-6">
          {loading ? (
            ...
            filteredRestaurants.map((res) => (
              <div 
                key={res._id} 
                className="bg-white p-4 rounded-[2rem] flex gap-5 shadow-[0_20px_50px_rgba(0,0,0,0.02)] border border-gray-50 hover:shadow-[0_30px_60px_rgba(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-500 cursor-pointer active:scale-95 group"
              >
